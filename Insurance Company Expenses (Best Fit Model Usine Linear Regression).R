...
colnames(datas)
datas= subset(datas, select = -c(NA.2))
datas= subset(datas, select = -c(NA.3))
datas= subset(datas, select = -c(NA.4))
datas= subset(datas, select = -c(NA.5))
datas= subset(datas, select = -c(NA.6))
datas= subset(datas, select = -c(NA.1))
datas= subset(datas, select = -c(15))
datas= subset(datas, select = -c(NA.7))
colnames(datas)

## Now data is ready for its Modeling

attach(datas)
datas$Moneytary_Assets = (LIQUIDRATIO*EXPENSES)
colnames(datas)
...
map(~ lm(Moneytary_Assets~ EXPENSES, data = datas))


simple.fit= lm(Moneytary_Assets~LONGLOSS, data= datas)
summary(simple.fit)
plot(simple.fit)
...
multi.fit=lm(Moneytary_Assets~LONGLOSS+SHORTLOSS, data = datas)
summary(multi.fit)
plot(multi.fit)


## Model 2
simple.fit= lm(TOTAL_ASSET~RBC, data= datas)
summary(simple.fit)
plot(simple.fit)
...
plot(lm(TOTAL_ASSET~RBC, data = datas), pch= 19, col= "red")
abline(lm(TOTAL_ASSET~RBC), col= "black")
cor(TOTAL_ASSET,RBC)
...
## Model 3
multi.fit= lm(EXPENSES~Moneytary_Assets+TOTAL_ASSET+LONGLOSS+SHORTLOSS+EXPENSES+GPWPERSONAL+GPWCOMM+STAFFWAGE+AGENTWAGE
              +RBC, data=datasa)
...
## Model 4 (Total asset and Gross premium commercial)
cor(TOTAL_ASSET,GPWCOMM)
plot(lm(TOTAL_ASSET~GPWCOMM, data = datas),pch=19, col= "blue")
multi.fit= lm(TOTAL_ASSET~GPWCOMM+GPWPERSONAL+RBC+EXPENSES)
summary(multi.fit)
plot(multi.fit)
plot(TOTAL_ASSET~GPWCOMM,pch=19, col= "blue")
abline(lm(TOTAL_ASSET~GPWCOMM, data = datas), col= "red")

plot(lm(TOTAL_ASSET~GPWPERSONAL, data = datas), pch=19, col= "blue")
abline(lm(TOTAL_ASSET~GPWPERSONAL, data = datas), col= "red")
...
hist(CASH,col = "blue", main="Histogram of Cash and Gross Premium Personal",xlab= "CASH + Gross Premium Personal" ,ylab="Freq")
hist(GPWPERSONAL,col = "red", main="Histogram of Gross Premium Personal", ylab="Freq", add= TRUE)
